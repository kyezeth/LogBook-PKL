        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ auth()->user()->isAdmin() ? route('admin.dashboard') : route('member.dashboard') }}" class="flex items-center space-x-2">
                        <img src="{{ asset('images/injourney-logo.png') }}" alt="Injourney Airports" class="h-9">
                    </a>
                </div>
                ...
                <div class="hidden space-x-1 sm:-my-px sm:ms-8 sm:flex">
                    ...
                    @else
                        ...
                        <x-nav-link :href="route('member.activities.index')" :active="request()->routeIs('member.activities.*')">
                            <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path>
                            </svg>
                            Kegiatan
                        </x-nav-link>
                        <x-nav-link :href="route('member.schedule.index')" :active="request()->routeIs('member.schedule.*')">
                            <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                            </svg>
                            Jadwal
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-amber-200 text-sm leading-4 font-medium rounded-xl text-slate-600 bg-white hover:bg-amber-50 hover:text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 transition ease-in-out duration-150 max-w-[200px]">
                            <img src="{{ Auth::user()->profile_photo_url }}" alt="{{ Auth::user()->name }}" class="w-7 h-7 rounded-full mr-2 flex-shrink-0 object-cover">
                            <div class="text-left min-w-0 flex-1">
                                <div class="text-sm font-medium truncate">{{ Str::limit(Auth::user()->name, 15) }}</div>
                                <div class="text-xs text-slate-400">{{ Auth::user()->isAdmin() ? 'Admin' : 'Siswa' }}</div>
                            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @else
                ...
                <x-responsive-nav-link :href="route('member.activities.index')" :active="request()->routeIs('member.activities.*')">
                    Kegiatan
                </x-responsive-nav-link>
            @endif
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-slate-200">
            ...
            <div class="mt-3 space-y-1">
                <x-responsive-nav-link :href="auth()->user()->isAdmin() ? route('admin.profile.edit') : route('member.profile.edit')">
                    Profil
                </x-responsive-nav-link>
